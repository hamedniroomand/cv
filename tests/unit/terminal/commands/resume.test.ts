...
describe('whoami', () => {
  it('prints identity, links and the hint, then navigates top', async () => {
    ...
    expect(out).toContain("Type 'help', run 'menu' for the guided mode — or just read the panel →");
...
describe('open', () => {
  ...
  it('lists targets on unknown input', async () => {
    ...
    expect(term.text()).toMatch(/github, linkedin, email, cpm, cue, waverune, kitdev, pdf/);
