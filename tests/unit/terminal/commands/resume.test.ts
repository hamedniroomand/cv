...
describe('whoami', () => {
  it('prints identity, links and the hint, then navigates top', async () => {
    ...
    const hrefs = term.lines.flatMap(l => l.spans).map(sp => sp.href).filter(Boolean)
...
describe('open', () => {
  ...
  it('completes targets', () => {
    const term = makeShell(commands)
    expect(completeLine('open gi', term.completion)).toEqual({ line: 'open github ', candidates: ['github'] })
  })
})
