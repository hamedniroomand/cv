...
describe('wc', () => {
  ...
  it('prints each file and a total for multiple files', async () => {
    const term = makeShell(commands);
    expect(
      (await term.exec('wc -l experience/acme/README.md experience/globex/README.md')).code,
    ).toBe(0);
    expect(term.text()).toBe(
      ['6 experience/acme/README.md', '5 experience/globex/README.md', '11 total'].join('\n'),
    );
  });
