...
describe('parse', () => {
  ...
  it('parses pipelines', () => {
    expect(parse('cat skills.json | jq .').segments.map(s => s.argv)).toEqual([['cat', 'skills.json'], ['jq', '.']])
  })

  it('extracts sudo prefix', () => {
    expect(parse('sudo cat .secrets')).toEqual({ segments: [{ argv: ['cat', '.secrets'], sudo: true }] })
