...
const cmd = (name: string, complete?: Command['complete']): Command => ({
  name,
  description: '',
  usage: name,
  run: () => 0,
...
describe('completeLine', () => {
  ...
  it('completes paths by default', () => {
    expect(completeLine('cat ab', ctx)).toEqual({
      line: 'cat about.md ',
      candidates: ['about.md'],
    });
    expect(completeLine('cat exp', ctx).line).toBe('cat experience/');
    expect(completeLine('cat experience/acme/', ctx).candidates).toEqual([
      'experience/acme/README.md',
      'experience/acme/highlights/',
    ]);
  ...
  it('completes after a pipe', () => {
    expect(completeLine('cat about.md | cl', ctx)).toEqual({
      line: 'cat about.md | clear ',
      candidates: ['clear'],
    });
  });
});
