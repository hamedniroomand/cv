...
const msg = {
  ...
  message: 'Hello there, long enough. @everyone',
};
...
describe('sendContact (discord)', () => {
  it('logs when no webhook is configured', async () => {
    ...
    expect(
      await sendContact(msg, { discordWebhookUrl: '' }, fetchImpl as unknown as typeof fetch),
    ).toBe('logged');
  ...
  it('posts an embed to the webhook with mentions disabled', async () => {
    ...
    expect(
      await sendContact(msg, { discordWebhookUrl: hook }, fetchImpl as unknown as typeof fetch),
    ).toBe('sent');
  ...
  it('truncates very long messages to the embed limit', async () => {
    const fetchImpl = vi.fn(async () => new Response('{}', { status: 200 }));
    await sendContact(
      { ...msg, message: 'x'.repeat(5000) },
      { discordWebhookUrl: hook },
  ...
  it('throws when Discord rejects', async () => {
    ...
    await expect(
      sendContact(msg, { discordWebhookUrl: hook }, fetchImpl as unknown as typeof fetch),
    ).rejects.toThrow(/429/);
  });
});
