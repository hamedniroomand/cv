...
describe('verifyTurnstile', () => {
  ...
  it('rejects a missing token without calling cloudflare', async () => {
    ...
    expect(await verifyTurnstile({ ...cfg, token: undefined }, fetchImpl as unknown as typeof fetch)).toBe(false)
    expect(await verifyTurnstile({ ...cfg, token: '' }, fetchImpl as unknown as typeof fetch)).toBe(false)
