  ...
  it('returns one message per invalid field and nothing for a valid form', () => {
    ...
    expect(
      contactFieldErrors({
        ...
        message: 'Hello there, long enough.',
      }),
    ).toEqual({});
  });
});
...
describe('issuesToFieldErrors', () => {
  it('maps the API issue tree to first messages and ignores garbage', () => {
    const issues = {
      email: { errors: ['email must be valid', 'too long'] },
      message: { errors: ['message must be at least 10 characters'] },
    };
...
describe('contactFailureFeedback', () => {
  it('keeps field errors and clears the form-level message', () => {
    const feedback = contactFailureFeedback({
      message: 'Invalid',
      issues: { email: { errors: ['email must be valid'] } },
    });
  ...
  it('surfaces honeypot and captcha errors as the form-level message', () => {
    expect(
      contactFailureFeedback({ issues: { website: { errors: ['invalid submission'] } } }).message,
    ).toBe('invalid submission');
    expect(
      contactFailureFeedback({ issues: { turnstileToken: { errors: ['captcha failed'] } } })
        .message,
    ).toBe('captcha failed');
  });
