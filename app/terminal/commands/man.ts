...
function manHamed(ctx: CommandContext): void {
  ...
  if (latest && role)
    description.push('', `Currently: ${role.title} at ${latest.company} (${formatRange(role.start, role.end)}).`)

  out.line('HAMED(1)                       User Commands                       HAMED(1)', 'dim')
  out.line()
  section(out, 'NAME', [`hamed - ${profile.title}`])
  section(out, 'SYNOPSIS', [`hamed [--${skills.categories.map(category => category.id).join('] [--')}]`])
