...
function directorySources(ctx: CommandContext, path: string): Source[] {
  ...
  ctx.fs.walk(path, (abs, node) => {
    if (node.type !== 'file' || isHiddenPath(abs)) return;
    try {
      sources.push({
        label: `${prefix}${abs.slice(base.length + 1)}`,
        text: ctx.fs.readFile(abs, { sudo: ctx.sudo }),
      });
    } catch (err) {
...
function pathSources(ctx: CommandContext, path: string, recursive: boolean): Source[] {
  try {
    ...
    if (recursive) return directorySources(ctx, path);
...
function grepSource(ctx: CommandContext, source: Source, opts: GrepOptions): number {
  ...
  splitLines(source.text).forEach((line, index) => {
    ...
    if (!opts.regex.test(line)) return;
    count++;
    if (opts.countOnly) return;
    if (label !== null) ctx.stdout.write(`${label}:`, 'dim');
    if (opts.numbered) ctx.stdout.write(`${index + 1}:`, 'dim');
    ctx.stdout.raw(highlight(line, opts.regex));
    ctx.stdout.line();
  });
  if (opts.countOnly) ctx.stdout.line(label !== null ? `${label}:${count}` : String(count));
  return count;
}
