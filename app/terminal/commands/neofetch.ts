...
const COLOUR_STYLES: LineStyle[] = ['dim', 'error', 'success', 'accent', 'plain', 'prompt', 'accent', 'plain']
...
function infoLines(ctx: CommandContext): InfoLine[] {
  const skillCount = ctx.cv.skills.categories.reduce((count, category) => count + category.items.length, 0)
  return [
