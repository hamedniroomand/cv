...
function printLong(ctx: CommandContext, nodes: FsNode[]): void {
  for (const node of nodes) {
    ...
    ctx.stdout.write(
      `${permissions(node)}  1 hamed hamed ${String(size).padStart(6)} ${formatDate(node.mtime)} `,
    );
...
export default {
  ...
  run(argv, ctx) {
    ...
    paths.forEach((path, index) => {
      ...
      if (paths.length > 1) {
        if (index > 0) ctx.stdout.line();
        ctx.stdout.line(`${path}:`);
      }
      print(ctx, node.type === 'file' ? [node] : ctx.fs.readdir(path, { all }));
    });

    if (positionals.length === 1) navigateFor(ctx, positionals[0]!);
    return code;
  },
} satisfies Command;
