...
export default {
  ...
  run(argv, ctx) {
    ...
    if (flags.has('pdf')) {
      ...
      ctx.stdout.line('Downloading hamed-niroomand-cv.pdf…')
      return 0
    }
    if (flags.has('json')) {
    ...
    const top = skills.categories.slice(0, 3).flatMap(c => c.items.slice(0, 3).map(i => i.name))
