...
function htmlToText(line: string): string {
  ...
  if (heading) return `${'#'.repeat(Number(heading[1]))} ${htmlToText(heading[2]!)}`;
  return line
...
function mergePlain(spans: Span[]): Span[] {
  ...
  for (const span of spans) {
    ...
    if (prev && !prev.style && !prev.href && !span.style && !span.href) prev.text += span.text;
    else out.push({ ...span });
  }
  return out;
}
...
export function renderInline(text: string): Span[] {
  ...
  for (const match of text.matchAll(INLINE)) {
    ...
    if (href !== undefined) push({ text: label!, style: 'accent', href });
    else if (code !== undefined) push({ text: code, style: 'accent' });
    else if (strong !== undefined) push({ text: strong, style: 'accent' });
    else push({ text: emphasis! });
...
function blockSpans(line: string): Span[] {
  ...
  if (heading) return [{ text: heading[1]!, style: 'accent' }];
  if (RULE_LINE.test(line)) return [RULE];
  const item = LIST_ITEM.exec(line);
  if (item) return [BULLET, ...renderInline(item[1]!)];
  const quote = QUOTE_LINE.exec(line);
  if (quote) return [QUOTE, ...renderInline(quote[1]!)];
  return renderInline(line.trim());
}

function pushBlank(lines: Span[][]): void {
  if (lines.length > 0 && lines[lines.length - 1]!.length > 0) lines.push([]);
}
...
export function renderMarkdown(source: string): Span[][] {
  ...
  for (const raw of source.replace(LINE_BREAK, '\n').split('\n')) {
    ...
    if (line.startsWith('```')) {
      ...
      if (fence === 'mermaid') lines.push([DIAGRAM]);
      continue;
    }
    ...
    if (HTML_TAG.test(line)) {
      ...
      if (text !== '') lines.push(blockSpans(text));
      continue;
    }
    if (line === '') pushBlank(lines);
    else lines.push(blockSpans(line));
  }
