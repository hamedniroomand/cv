  ...
  <dialog ref="dialog" class="modal" aria-labelledby="contact-title" @cancel.prevent="close" @click.self="close">
    <form class="modal__form" novalidate @submit.prevent="onSubmit">
      ...
      <template v-if="state !== 'sent'">
        <ContactField v-model="form.name" name="name" label="name:" :error="errors.name" type="text" required maxlength="100" autocomplete="name" />
        <ContactField v-model="form.email" name="email" label="email:" :error="errors.email" type="email" required maxlength="200" autocomplete="email" />
        <ContactField v-model="form.message" name="message" label="message:" :error="errors.message" multiline required minlength="10" maxlength="5000" rows="5" />
        <input v-model="form.website" class="visually-hidden" type="text" name="website" tabindex="-1" autocomplete="off" aria-hidden="true">
        <div v-if="siteKey" class="modal__row">
        ...
        <div class="modal__actions">
          <button type="submit" class="btn" :disabled="sending || !verified">
