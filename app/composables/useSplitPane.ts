...
function applyPanel(open: boolean): void {
  if (open) delete document.documentElement.dataset.panel;
  else document.documentElement.dataset.panel = 'closed';
}
...
export function useSplitPane() {
  ...
  if (import.meta.client) {
    onMounted(() => {
      ...
      if (readStorage(SPLIT_PANEL_KEY) === 'closed') panelOpen.value = false;
    });
  }
